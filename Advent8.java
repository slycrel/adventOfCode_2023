...
public class Advent8
{
	...
	static void doPart2(String directions)
	{
		...
		do
		{
			...
			// walk forward one step for each node, saving the new nodes along the way
			var list = new ArrayList<Node>();
			for (var node : starts)
			{
				if ("L".equals(nextStep))
					list.add(network.get(node.left));
				else
					list.add(network.get(node.right));
			}
			// replace the starts with the new step list
			starts = list;
	...
	private static boolean allOnZ(ArrayList<Node> nodes)
	{
		for (var node : nodes)
		{
	...
	private static ArrayList<Node> BuildPart2Starts()
	{
